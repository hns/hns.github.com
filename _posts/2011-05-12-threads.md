...
never have to worry about concurrency problems at all. Functional means that
...
convenient way to use these modules is of course to define the callback
functions as closures in place.
...
synchronizes on the lock argument. Synchronize means that at each point in time
at most one thread may run a synced function on the lock object. When another
thread tries to invoke a synced function on the same object, it will be blocked
until the first thread exits its synced function.
...
We're adding a `lock` object at the same scope as the data we want to protect.
...
Thus, this version of `doSomething` always returns "original".

Locking is a very complicated topic that is impossible to cover in any depth
here. I'll just say that course locking is generally easier and less risky
but also less performant than fine grained locking. When dealing with multiple
locks it's a good idea to always acquire them in the same order to avoid
deadlocks.
...
As we have seen, the fact that JavaScript functions are closures over mutable
shared state is maybe the biggest stumbling block when trying to implement safer
threading constructs such as Erlang/Scala like Actors. The way it is, the
smallest unit of code we can deal with in a side-effect free way is the module,
which is not very satisfactory.

Maybe we could take a deeper look at Clojure for inspiration. Clojure's [agents]
might be a better fit for JavaScript than actors. Instead of sending data to a
piece of code, you send code to a piece of data (so to speak). Also, Clojure's
persistent data structures look like a nice fit for Ringo.

The biggest problem with threads is still that they are not visible when looking
at the code. You just have to know about them. In that light I should probably
have written this article a long time ago.
